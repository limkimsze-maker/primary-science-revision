...
const SCI_BASE='psleScience180_book_master_v1';
const PROC_BASE='psleScience_process_skills_v1';
let allowOnce='';
const tokenKey=s=>`psleScience_cloud_auth_token_${s}`;
const profileKey=(base,s)=>`${base}__profile_${s}`;
const nameOf=s=>s==='jerry'?'Jerry':'Javis';
const parse=(x,f={})=>{try{return JSON.parse(x||'')||f}catch(_e){return f}};
...
async function sessionToken(student){
 ...
 try{
  ...
  if(!r.ok){localStorage.removeItem(tokenKey(student));return ''}
  const d=await r.json();
  if(d?.ok&&d?.student===student)return token;
 }catch(_e){}
 return '';
}

async function syncBeforeEnter(student,token){
 const states={
  [SCI_BASE]:parse(localStorage.getItem(profileKey(SCI_BASE,student)),{}),
  [PROC_BASE]:parse(localStorage.getItem(profileKey(PROC_BASE,student)),{items:{}})
 };
 const r=await fetch(AUTH+'/progress/sync',{
  method:'POST',
  headers:{'content-type':'application/json',authorization:`Bearer ${token}`},
  body:JSON.stringify({states})
 });
 const d=await r.json().catch(()=>({}));
 if(!r.ok)throw new Error(d?.error||'Could not sync progress.');
 if(d?.states?.[SCI_BASE])localStorage.setItem(profileKey(SCI_BASE,student),JSON.stringify(d.states[SCI_BASE]));
 if(d?.states?.[PROC_BASE])localStorage.setItem(profileKey(PROC_BASE,student),JSON.stringify(d.states[PROC_BASE]));
 localStorage.setItem(`psleScience_cloud_last_sync_${student}`,String(d?.syncedAt||Date.now()));
 return d;
}
...
function loginModal(student,button){
 ...
 form.onsubmit=async e=>{
  ...
  try{
   ...
   go.textContent='Combining progress…';
   await syncBeforeEnter(student,d.token);
...
// Capture profile selection before the original page handler. Every successful
// sign-in merges this device's progress with D1 before the trainer opens.
document.addEventListener('click',async e=>{
 ...
 const token=await sessionToken(student);
 if(token){
  try{
   b.textContent='Combining progress…';
   await syncBeforeEnter(student,token);
   b.disabled=false;b.textContent=old;allowOnce=student;b.click();return;
  }catch(_e){
   b.disabled=false;b.textContent=old;
   // Keep the valid session, but ask again so the user gets a clear sync error if it persists.
  }
...
function enhance(){
 const cards=[...document.querySelectorAll('.profile')];if(!cards.length)return;
 cards.forEach(card=>{
  if(!card.querySelector('.sm-progress')){
   const metrics=[...card.querySelectorAll('.metric')];const knownMetric=metrics.find(m=>/Total Known/i.test(m.textContent));const known=Number((knownMetric?.querySelector('b')?.textContent||'0').split('/')[0])||0;
   const next=[1,5,10,25,50,75,100,125,150,175,192].find(x=>x>known)||192;
   const title=known>=192?'Mastery Complete':known>=150?'PSLE Science Ace':known>=100?'Science Strategist':known>=60?'Science Specialist':known>=30?'Scientist':known>=15?'Investigator':known>=5?'Explorer':'Starter';
   const msg=known===0?'Start with one item. One correct Application question makes it Known.':known<10?'Each Known item is one less idea to worry about in revision.':known<50?'Momentum is building. Keep turning recall into application.':known<100?'Strong foundation. Keep converting Science ideas into PSLE answers.':known<150?'You have mastered a large part of the bank. Keep closing the remaining gaps.':known<192?'Final stretch. Focus on the Not done list and finish strongly.':'All 192 items are Known. Revisit weak areas whenever you want extra transfer practice.';
   const el=document.createElement('div');el.className='sm-progress';el.innerHTML=`<div class="sm-row"><b>${title}</b><span>${known*100} Mastery XP</span></div><div class="sm-track"><i style="width:${Math.min(100,known/192*100)}%"></i></div><div class="sm-copy">${msg}</div><div class="sm-next">${known>=192?'🏅 192/192 mastered':`${next-known} more to reach the ${next}-Known milestone`}</div>`;card.querySelector('.profile-actions')?.insertAdjacentElement('beforebegin',el)
  }
  if(!card.querySelector('.cloud-note')){const n=document.createElement('div');n.className='cloud-note';n.textContent='☁ Password-protected cloud progress';card.querySelector('.status')?.insertAdjacentElement('afterend',n)}
 });
