...
function install(){
  ...
  const note=document.createElement('div');note.className='small';note.style.marginTop='6px';note.innerHTML='AI marks the <b>idea</b>, not exact wording. A response that already contains the reviewed model answer is accepted automatically. For fair-test questions it checks whether the exact controlled variable is linked to the changed and measured variables.';row.insertAdjacentElement('afterend',note);
...
function recordCorrect(box,msg){
  ...
  setTimeout(()=>{if(box){box.className='fb good';box.innerHTML='<b>🤖 Correct ✅</b><br>'+esc(msg)}},0);
}
async function mark(){
  ...
  // Deterministic safety check: if the pupil's response already contains the complete reviewed
  // model answer, it cannot be marked wrong by a later generic AI classification.
  const aNorm=norm(answer),mNorm=norm(variant.model||'');
  if(mNorm&&aNorm.includes(mNorm)){
    recordCorrect(box,'Your answer contains the complete reviewed model answer. Extra scientifically correct explanation does not make it wrong.');
  ...
  try{
    ...
    if(ok){recordCorrect(box,detail||'The required process-skill idea is present.')}
