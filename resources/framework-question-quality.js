...
const META={
 F1:{n:1,name:'Change → Effect'},F2:{n:2,name:'Mechanism / Process'},F3:{n:3,name:'Evidence / Data → Explanation'},
 F4:{n:4,name:'Comparison → Explanation'},F5:{n:5,name:'Structure / Feature → Function'},F6:{n:6,name:'Experimental / Process-skill'}
};

function classify(q){
 const raw=String(q||''),t=norm(raw);if(!t)return null;
 // F6: explicit experimental design/quality language.
 ...
 // F4: an explicit comparison should not be swallowed by evidence words such as results/observations.
 ...
 // F5: structure/feature/property → what it enables → function/advantage.
 ...
 // F3: the pupil must interpret supplied evidence/data/observations to reach a conclusion.
 ...
 // F2: process/pathway/sequence/mechanism.
 ...
 // F1: a changed condition causes an effect/result. Explain/Why is the broad fallback.
...
function candidates(e,id){
 const out=[];
 const push=(q,model,source)=>{q=String(q||'').trim();if(!q||BAD.test(q))return;out.push({q,model:String(model||e?.phrase||'').trim(),source})};
...
function scoreCandidate(c,e){
 let s=0;const t=norm(c.q),topic=norm(e?.topic||''),phrase=norm(e?.phrase||'');
 topic.split(' ').filter(w=>w.length>4).forEach(w=>{if(t.includes(w))s+=7});
 phrase.split(' ').filter(w=>w.length>5).slice(0,10).forEach(w=>{if(t.includes(w))s+=2});
 const words=t.split(' ').length;if(words>=8&&words<=55)s+=20;
 if(/\b(explain|why|compare|using|predict|what .* show|trace|how)\b/.test(t))s+=12;
 if(c.source.startsWith('Audited'))s+=12;
 return s;
}
function fallbackFor(e,f){
 const topic=String(e?.topic||'the Science concept').replace(/[—–]/g,' ').replace(/\s+/g,' ').trim().toLowerCase();
 const cue=String(e?.phrasePrompt||'').trim().replace(/\?$/,'');
 if(f==='F2'&&/^(how|what happens)/i.test(cue))return `Explain ${cue.charAt(0).toLowerCase()+cue.slice(1)}.`;
 if(f==='F5'&&/^how/i.test(cue))return `Explain ${cue.charAt(0).toLowerCase()+cue.slice(1)}.`;
 if(f==='F3')return `A pupil makes an observation related to ${topic}. Explain what the observation would show using the Science idea in this concept.`;
 if(f==='F4')return `Compare two cases involving ${topic} and explain the relevant difference using the Science concept.`;
 if(f==='F6')return `A pupil investigates ${topic}. State what should be changed, what should be measured and one relevant variable that should be kept the same.`;
 if(f==='F5')return `Explain how the relevant feature in ${topic} helps it carry out its function.`;
 return `A condition affecting ${topic} changes. Explain how the change can affect the result.`;
}
function choose(item,e){
 ...
 // Natural, classifiable questions are safer than forcing a question into a framework it does not use.
 const natural=cs.filter(c=>c.f).sort((a,b)=>scoreCandidate(b,e)-scoreCandidate(a,e));
 if(natural.length)return natural[0];
 // This should be rare after the full audit. Keep a final one-to-one question rather than leaving an item blank.
 return{q:fallbackFor(e,item.framework),model:e?.phrase||'',source:'Framework fallback',f:item.framework};
}
...
function patchCountUI(api){
 const c=actualCounts(api),sel=$('fcFrameworkFilter');
 if(sel){const keep=/^F[1-6]$/.test(sel.value)?sel.value:'all';sel.innerHTML=`<option value="all">All 6 frameworks — 192</option>`+Object.keys(c).map(f=>`<option value="${f}">${f} ${META[f].name} — ${c[f]}</option>`).join('');sel.value=keep}
 const map=$('fcMap');if(map)map.innerHTML=`<b>192 linked PSLE-style framework questions</b><br>F1 ${c.F1} · F2 ${c.F2} · F3 ${c.F3} · F4 ${c.F4} · F5 ${c.F5} · F6 ${c.F6}<br><small>Each question remains linked to its flashcard. Existing Known progress is unchanged.</small>`;
 return c;
}
function patchCurrent(){
 ...
 try{
  ...
  const card=$('frameworkCoach');if(card){const b=card.querySelector('.fc-best b');if(b)b.textContent=`${item.framework} — ${META[item.framework].name}`;card.dataset.fr=item.framework}
 }catch(_e){}
}
function apply(){
 ...
 for(let id=1;id<=180;id++){
  ...
  const pick=choose(item,e),f=pick.f||classify(pick.q)||item.framework;
  item.framework=f;item.question=pick.q;item.model=pick.model||e.phrase||'';item.source=pick.source;
 ...
 const c=patchCountUI(api);api.auditedCounts=()=>({...c});
 patchCurrent();d.dispatchEvent(new Event('psle-framework-question-quality-updated'));return true;
}
