...
function installUI(){
  ...
  note.innerHTML='AI checks the answer for the required <b>Science concept</b> and, when relevant, <b>D/E → S/R → L/R</b>. Scientifically equivalent wording is accepted. The model answer stays hidden until you choose <b>Show Model Answer</b>.';
...
function collectFeedback(d){if(!d)return '';let parts=[];for(const k of ['feedback','missing','strengths','reason','explanation','message']){const v=d[k];if(typeof v==='string'&&v.trim())parts.push(v.trim())}return [...new Set(parts)].join(' ')}
