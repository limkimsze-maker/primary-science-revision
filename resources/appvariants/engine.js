...
const PAPER_EVIDENCE={13:'Q32a · remediation completed',14:'Q35b',45:'Q33b',46:'Q33b',50:'Q33c · remediation completed',62:'Q34b/d · remediation completed',65:'Q33a · remediation completed',70:'Q32d · remediation completed',102:'Q36',106:'Q37',107:'Q37',140:'Q38a',141:'Q38a',143:'Q38c',145:'Q38b',149:'Q40a',150:'Q40a',166:'Q41a/c · remediation completed',169:'Q41a/c · remediation completed',174:'Q39a',176:'Q39c'};
function applyPaperProfile(){try{if(localStorage.getItem(PAPER_PROFILE_KEY)==='done'||typeof rec!=='function'||typeof save!=='function')return;PAPER_GREEN.forEach(id=>{if(id<1||id>BANK.length)return;let x=rec(id-1);if(x.prePaperOverride===undefined)x.prePaperOverride=x.override||'auto';x.override='green';x.paperProfile='strong';x.paperSource=PAPER_SOURCE;x.paperEvidence=PAPER_EVIDENCE[id]||''});PAPER_RED.forEach(id=>{if(id<1||id>BANK.length)return;let x=rec(id-1);if(x.prePaperOverride===undefined)x.prePaperOverride=x.override||'auto';x.override='red';x.paperProfile='focus';x.paperSource=PAPER_SOURCE;x.paperEvidence=PAPER_EVIDENCE[id]||''});save();localStorage.setItem(PAPER_PROFILE_KEY,'done')}catch(e){console.warn('Could not apply past-paper profile',e)}}
applyPaperProfile();
...
BANK.forEach(e=>{
 const g=[{question:e.phrasePrompt,kind:'Recall',modelApplicationAnswer:e.modelApplicationAnswer||e.phrase||'',source:e.bookRef||''}];
 const sp=(V[e.id]||[]).map(x=>({question:x.q,kind:x.k||'Transfer',modelApplicationAnswer:x.model||e.modelApplicationAnswer||e.phrase||'',source:x.source||'',paperBacked:x.paperBacked===true}));
 e.applicationVariants=uq([...g,...sp]).slice(0,7).map(x=>({question:x.question,kind:x.kind,rubric:e.rubric,modelApplicationAnswer:x.modelApplicationAnswer||e.modelApplicationAnswer||e.phrase||'',source:x.source||'',paperBacked:x.paperBacked===true}));
});
...
function model(){let i=current(),e=BANK[i],v=vv(i),b=$('modelAppBox');if(b.classList.contains('hide')){const source=v.source?`<div class="small" style="margin-top:6px"><b>Question source:</b> ${esc(v.source)}</div>`:'';b.innerHTML=`<b>Model answer for this exact question</b><br><strong>${esc(v.modelApplicationAnswer)}</strong>${source}<div class="small" style="margin-top:8px">Science core: ${esc(e.bookRef)}. Scientifically equivalent wording is acceptable.</div>`;b.classList.remove('hide');$('modelAppBtn').textContent='Hide Science Core'}else{b.classList.add('hide');$('modelAppBtn').textContent='Show Science Core'}}
...
let sub=document.querySelector('header .sub');if(sub)sub.innerHTML='<b>180 re-audited Science concepts</b> · book-backed core · <b>2026 prelim-source questions first</b> · natural PSLE-style transfer when no suitable paper item exists';
let note=app.querySelector('.mode-note');if(note)note.innerHTML='<b>Written Application:</b> the trainer uses a suitable question from the uploaded 2026 P6 prelim papers first. Where the original paper depends on a missing diagram/table, the context is adapted without changing the Science demand. If no suitable paper item matches the flashcard, a natural authored PSLE-style question is used. Exact wording is not required.';
const baseCheckPhrase=checkPhrase;
checkPhrase=function(){const shouldJump=$('phraseMode').value==='exact'&&norm($('phraseAnswer').value)===norm(BANK[current()].phrase);baseCheckPhrase();if(shouldJump){setPane('app');const box=$('appFeedback');if(box){box.className='fb good';box.innerHTML='<b>Exact recall correct ✅</b><br>Now answer the application question for the same concept.'}$('appAnswer').focus();window.scrollTo({top:0,behavior:'smooth'})}};
