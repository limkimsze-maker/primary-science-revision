...
async function finalizeSession(s){
 if(!s||s.finalizing)return;s.finalizing=true;s.active=false;s.manualStop=true;clearTimeout(s.restartTimer);clearTimeout(s.maxTimer);commitCycle(s);
 const raw=tidy(s.accumulated);if(!raw){endUI(s,'No speech was captured. Tap the microphone and try again.');return}
 ...
 s.box.value=finalText;s.box.dataset.voiceUsed='1';s.box.dataset.rawTranscript=raw;s.box.dataset.cleanedTranscript=result.cleaned;s.box.dataset.voiceUserEdited='0';s.box.dispatchEvent(new Event('input',{bubbles:true}));renderStatus(s,result);s.box.focus();endUI(s,s.status.innerHTML);
}
...
function startCycle(s){
 if(!s.active||s.manualStop||s.finalizing)return;
 ...
 rec.onstart=()=>{if(s.active&&!s.manualStop){s.btn.textContent='⏹ Stop Listening';s.btn.classList.add('speech-listening');s.status.textContent='Listening… Tap Stop Listening when you are finished.'}};
 rec.onresult=e=>{if(!s.active||s.manualStop)return;const alts=collectAlternatives(e);if(alts[0]){s.cycleText=alts[0];alts.slice(1).forEach(x=>s.alternatives.add(x));livePreview(s)}};
 rec.onerror=e=>{if(e.error==='no-speech'&&s.active&&!s.manualStop)return;const map={'not-allowed':'Microphone permission was blocked. Allow microphone access, then try again.','service-not-allowed':'Speech recognition is blocked by this browser/device.','audio-capture':'No microphone was detected.','network':'Speech recognition could not connect. Check the internet connection and try again.'};s.errorMessage=map[e.error]||`Speech recognition error: ${e.error||'unknown error'}`};
 rec.onend=()=>{
  ...
  // Browser ended because of a pause. Keep the overall session alive until the user presses Stop Listening.
...
function startSession(box,btn,status,row){
 ...
 const wasVoice=box.dataset.voiceUsed==='1'&&box.dataset.voiceUserEdited!=='1';
 const s={id:++sessionSeq,box,btn,status,row,baseBefore:wasVoice?'':tidy(box.value||''),accumulated:'',cycleText:'',alternatives:new Set(),active:true,manualStop:false,finalizing:false,restartTimer:null,maxTimer:null,rec:null,errorMessage:''};
 currentSession=s;btn.textContent='⏹ Stop Listening';btn.classList.add('speech-listening');status.textContent='Listening… Tap Stop Listening when you are finished.';
 // Safety cap only; normal completion should be by the Stop Listening button.
...
function installOne(t){
 ...
 box.addEventListener('input',e=>{if(e.isTrusted)box.dataset.voiceUserEdited='1'},true);box.addEventListener('focus',()=>{if(!tidy(box.value)){delete box.dataset.voiceUsed;delete box.dataset.contextCorrections;delete box.dataset.cleanupSource}});
