...
function startCycle(s){
 ...
 rec.onstart=()=>{if(s.active&&!s.manualStop){s.btn.textContent='⏹ Stop Listening';s.btn.classList.add('speech-listening');s.status.textContent='Listening in UK English… Tap Stop Listening when you are finished.'}};
 rec.onresult=e=>{
...
function startSession(box,btn,status,row){
 ...
 currentSession=s;btn.textContent='⏹ Stop Listening';btn.classList.add('speech-listening');status.textContent=freshAfterClear?'Starting a fresh UK-English recording…':'Listening in UK English… Tap Stop Listening when you are finished.';
