...
function install(){
 ...
 let mode='all';
 const isExplain=()=>/\b(explain|why)\b/i.test(question.textContent||'');
 const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 function framework(){
  const b=document.getElementById('appExplainFrame');if(!b)return;
  const on=isExplain();b.classList.toggle('hide',!on);
  if(on)b.innerHTML='<b>Explain question</b><span><strong>D/E</strong> evidence from the question → <strong>S/R</strong> science reasoning → <strong>L/R</strong> link back to the result asked</span>';
 }
 ...
 function setMode(v){
  ...
  if(mode==='explain'){
   if(!seekExplain(true)){
    const fb=document.getElementById('appFeedback');if(fb){fb.className='fb warnbox';fb.textContent='No Explain question was found in the current queue.'}
   }
  }
  afterQuestionChange();
 }
 const bar=document.createElement('div');bar.id='appQuestionModeBar';bar.className='app-question-mode';bar.innerHTML='<b>Question type</b><button type="button" data-app-question-mode="all" class="active">All questions</button><button type="button" data-app-question-mode="explain">Explain · D/E → S/R → L/R</button>';
 ...
 modelBtn.onclick=async function(){
  ...
  modelBox.innerHTML=`<div class="app-answer-card your"><b>✍️ Your answer</b><div>${esc(student)}</div></div><div class="app-answer-card model"><b>✅ Model answer</b>${isExplain()?'<div class="app-mini-frame"><span>D/E</span>→<span>S/R</span>→<span>L/R</span></div>':''}<div><strong>${esc(model)}</strong></div><small>${fallback?'AI model could not be prepared, so the audited science core is shown. Adapt it to the question context.':'PSLE-style model for this question. Scientifically equivalent wording is acceptable.'}</small></div>`;
  modelBtn.disabled=false;modelBtn.textContent='Hide Model Answer';
 };
 const note=app.querySelector('.mode-note');if(note)note.innerHTML='<b>Written Application:</b> choose <b>All questions</b> or <b>Explain · D/E → S/R → L/R</b>. Write your own answer first. The <b>Model Answer</b> unlocks only after you have tried, and it will show your answer beside the model for comparison.';
 const style=document.createElement('style');style.textContent=`
 .app-question-mode{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:4px 0 10px;padding:10px 12px;border:1px solid #dbe3ee;border-radius:13px;background:#fff}.app-question-mode>b{font-size:13px;color:#475569}.app-question-mode button{font-weight:800}.app-question-mode button.active{background:#4338ca;color:#fff;border-color:#4338ca}.app-explain-frame{margin:7px 0 11px;padding:10px 12px;border:1px solid #c7d2fe;border-radius:12px;background:#eef2ff;color:#3730a3}.app-explain-frame>b{display:block}.app-explain-frame span{display:block;margin-top:4px;font-size:12px;color:#475569}.app-answer-compare{display:grid!important;grid-template-columns:1fr 1fr;gap:10px;background:transparent!important;border:0!important;padding:0!important}.app-answer-card{padding:13px;border-radius:13px;border:1px solid #dbe3ee;background:#fff;line-height:1.55}.app-answer-card>b{display:block;margin-bottom:7px}.app-answer-card.your{background:#f8fafc}.app-answer-card.model{background:#f0fdf4;border-color:#bbf7d0}.app-answer-card small{display:block;margin-top:8px;color:#64748b}.app-mini-frame{display:flex;align-items:center;gap:5px;margin-bottom:8px;color:#64748b;font-size:12px}.app-mini-frame span{background:#eef2ff;color:#3730a3;border-radius:999px;padding:3px 7px;font-weight:900}@media(max-width:720px){.app-answer-compare{grid-template-columns:1fr}.app-question-mode button{flex:1 1 100%}}
