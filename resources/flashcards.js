...
let list=[],pos=0,back=false,font=42,mode=false,speaking=false,currentUtterance=null,speechEpoch=0;
...
function currentKey(){return list.length?`${list[pos]}:${back?'answer':'question'}`:''}
function visibleText(){return back?($('fcAnswer')?.textContent||''):($('fcPrompt')?.textContent||'')}
function setReadButton(){const b=$('fcRead');if(b)b.textContent=speaking?'⏹ Stop Reading':'🔊 Read Again'}
function stopRead(){
 speechEpoch++;
 try{speechSynthesis.cancel()}catch(_e){}
 speaking=false;currentUtterance=null;setReadButton();
}
function exactUKVoice(){
 if(typeof speechSynthesis==='undefined')return null;
 const voices=speechSynthesis.getVoices?.()||[];
 const uk=voices.filter(v=>/^en[-_]GB$/i.test(v.lang||''));
 const preferred=['Sonia','Serena','Hazel','Libby','Susan','Martha','Kate','Emma','Amy','Female'];
 return uk.find(v=>preferred.some(n=>(v.name||'').toLowerCase().includes(n.toLowerCase())))||uk[0]||null;
}
function waitForUKVoice(){
 const now=exactUKVoice();if(now)return Promise.resolve(now);
 if(typeof speechSynthesis==='undefined')return Promise.resolve(null);
 return new Promise(resolve=>{
  let done=false;
  const finish=()=>{if(done)return;done=true;try{speechSynthesis.removeEventListener('voiceschanged',onVoices)}catch(_e){};resolve(exactUKVoice())};
  const onVoices=()=>{if(exactUKVoice())finish()};
  try{speechSynthesis.addEventListener('voiceschanged',onVoices,{once:false})}catch(_e){}
...
async function speakVisible(){
 ...
 stopRead();
 const epoch=speechEpoch,key=currentKey(),text=visibleText().trim();
 if(!text)return;
 const voice=await waitForUKVoice();
 if(epoch!==speechEpoch||key!==currentKey()||!mode||text!==visibleText().trim())return;
 const u=new SpeechSynthesisUtterance(text);currentUtterance=u;u.lang='en-GB';u.rate=.88;u.pitch=1;
 if(voice)u.voice=voice;
 u.onstart=()=>{if(epoch!==speechEpoch||key!==currentKey()||currentUtterance!==u){try{speechSynthesis.cancel()}catch(_e){};return}speaking=true;setReadButton()};
 const finish=()=>{if(currentUtterance!==u)return;speaking=false;currentUtterance=null;setReadButton()};
 u.onend=finish;u.onerror=finish;
 try{speechSynthesis.speak(u)}catch(_e){finish()}
}
function readCurrent(){if(speaking){stopRead();return}speakVisible()}
function rebuild(){stopRead();list=filters();pos=0;back=false;renderChips();render();if(mode&&list.length)speakVisible()}
function render(){
 ...
 const i=list[pos],e=BANK[i],p=state(i);card.dataset.cardIndex=String(i);card.dataset.side=back?'answer':'question';$('fcCounter').textContent=`${pos+1} / ${list.length}`;$('fcNumber').textContent=`#${e.id}`;$('fcTopic').textContent=e.topic;$('fcPrompt').textContent=e.phrasePrompt;$('fcAnswer').textContent=e.phrase;$('fcSource').textContent=`📘 ${e.bookRef}`;$('fcCategoryBadge').textContent=e.category;$('fcPriorityBadge').textContent=label(p);$('fcPriorityBadge').className=`fc-priority ${p}`;$('fcFace').textContent=back?'EXPLANATION':'QUESTION';card.classList.toggle('show-back',back);$('fcFlip').textContent=back?'Next Question →':'Show Answer →';$('fcTapHint').textContent=back?'Tap card again for the next question.':'Tap card to show the answer.';setReadButton();
}
function cardTap(){
 if(!list.length)return;stopRead();
 if(!back){back=true;render();speakVisible();return}
 pos=(pos+1)%list.length;back=false;render();speakVisible();
}
function move(d){if(!list.length)return;stopRead();pos=(pos+d+list.length)%list.length;back=false;render();if(mode)speakVisible()}
function shuffle(){stopRead();for(let i=list.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[list[i],list[j]]=[list[j],list[i]]}pos=0;back=false;render();if(mode&&list.length)speakVisible()}
...
function install(){
 ...
 <div class="fc-head"><div><h2>🃏 Explanation Flashcards</h2><p>See question → hear question in UK English → tap for answer → hear answer → tap for next question.</p></div><b id="fcSetCount"></b></div>
 ...
 <div id="fcCard" class="fc-card" tabindex="0"><div class="fc-meta"><b id="fcFace">QUESTION</b><span id="fcNumber"></span><span id="fcPriorityBadge"></span><span id="fcCategoryBadge" class="fc-category"></span><span class="fc-voice">🇬🇧 UK English</span></div><div id="fcTopic" class="fc-topic"></div><div class="fc-front"><div id="fcPrompt" class="fc-prompt"></div></div><div class="fc-back"><div id="fcAnswer" class="fc-answer"></div><div id="fcSource" class="fc-source"></div></div><div id="fcTapHint" class="fc-taphint">Tap card to show the answer.</div></div>
 <div class="fc-nav"><button id="fcPrev">← Previous</button><b id="fcCounter"></b><button id="fcRead">🔊 Read Again</button><button id="fcFlip" class="primary">Show Answer →</button><button id="fcNext" class="next">Next →</button></div>
 <div class="fc-tip"><b>Simple flow:</b> speech is locked to the text currently visible on the card, so the question and explanation cannot cross over.</div>`;header.insertAdjacentElement('afterend',panel);
 const style=document.createElement('style');style.textContent=`.fc-hide{display:none!important}.fc-panel{margin-top:14px;--fc-font:42px}.fc-head{display:flex;justify-content:space-between;gap:12px;align-items:flex-start;flex-wrap:wrap}.fc-head h2{font-size:clamp(28px,4vw,40px);margin:0}.fc-head p{color:#64748b;margin:5px 0;font-weight:700}.fc-head>b{background:#eef2ff;color:#3730a3;border-radius:999px;padding:8px 12px}.fc-chips{display:flex;gap:7px;overflow-x:auto;padding:12px 0 5px}.fc-chip{white-space:nowrap;padding:8px 10px;font-size:12px}.fc-chip.on{background:#4338ca;color:#fff;border-color:#4338ca}.fc-tools{display:flex;gap:8px;align-items:end;flex-wrap:wrap;margin:12px 0}.fc-tools label{display:flex;flex-direction:column;gap:4px;font-size:12px;font-weight:800;color:#475569}.fc-tools input{flex:1;min-width:240px}.fc-size{display:flex;align-items:center;gap:4px}.fc-size button{padding:8px 10px}.fc-size b{font-size:12px}.fc-card{min-height:430px;background:#fff;border:2px solid #c7d2fe;border-radius:26px;box-shadow:0 12px 35px #0f172a18;padding:62px clamp(20px,4vw,48px) 62px;display:flex;flex-direction:column;justify-content:center;position:relative;cursor:pointer;transition:transform .12s ease,border-color .12s ease}.fc-card:active{transform:scale(.995)}.fc-meta{position:absolute;top:18px;left:20px;right:20px;display:flex;gap:7px;flex-wrap:wrap;align-items:center;color:#64748b;font-size:12px}.fc-meta>b{color:#4338ca;letter-spacing:.1em}.fc-category,.fc-priority,.fc-voice{padding:5px 8px;border-radius:999px;font-weight:800}.fc-category{background:#eef2ff;color:#3730a3}.fc-voice{background:#eff6ff;color:#1d4ed8}.fc-priority.red{background:#fee2e2;color:#991b1b}.fc-priority.amber{background:#f8fafc;color:#475569}.fc-priority.green{background:#dcfce7;color:#166534}.fc-topic{text-align:center;font-size:clamp(20px,3vw,30px);font-weight:900;color:#334155;margin-bottom:18px}.fc-front,.fc-back{text-align:center}.fc-prompt{font-size:clamp(30px,5vw,58px);font-weight:900;line-height:1.22}.fc-back{display:none}.show-back .fc-front{display:none}.show-back .fc-back{display:block}.show-back{border-color:#86efac;background:linear-gradient(180deg,#fff,#f0fdf4)}.fc-answer{font-size:var(--fc-font);font-weight:900;line-height:1.32}.fc-source{margin-top:24px;color:#166534;font-size:14px;font-weight:700}.fc-taphint{position:absolute;left:18px;right:18px;bottom:18px;text-align:center;color:#64748b;font-weight:800;font-size:14px}.show-back .fc-taphint{color:#166534}.fc-nav{display:flex;justify-content:center;align-items:center;gap:9px;flex-wrap:wrap;margin-top:14px}.fc-nav>b{min-width:75px;text-align:center}.fc-tip{text-align:center;color:#64748b;font-size:13px;margin:16px auto}.fc-empty{background:#fff;border:1px solid #dbe3ee;border-radius:18px;padding:40px;text-align:center;font-weight:800}@media(max-width:650px){.fc-card{min-height:390px;padding:62px 18px 62px}.fc-answer{font-size:min(var(--fc-font),10vw)}.fc-tools>*{width:100%}.fc-tools input{min-width:0}.fc-size{width:auto}.fc-nav button{flex:1 1 40%}.fc-nav>b{width:100%;order:-1}}`;document.head.appendChild(style);
 tab.onclick=enter;$('phraseTab')?.addEventListener('click',leave);$('appTab')?.addEventListener('click',leave);$('fcCategory').onchange=rebuild;$('fcPriority').onchange=rebuild;$('fcSearch').oninput=rebuild;$('fcShuffle').onclick=()=>{list=filters();shuffle()};$('fcMinus').onclick=()=>setFont(font-4);$('fcPlus').onclick=()=>setFont(font+4);$('fcCard').onclick=cardTap;$('fcPrev').onclick=()=>move(-1);$('fcNext').onclick=()=>move(1);$('fcRead').onclick=readCurrent;$('fcFlip').onclick=cardTap;$('fcCard').onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();cardTap()}if(e.key==='ArrowLeft')move(-1);if(e.key==='ArrowRight')move(1)};renderChips();rebuild();setFont(font);return true;
}
