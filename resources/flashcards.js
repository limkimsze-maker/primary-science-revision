...
function stopRead(){
 ...
 const b=$('fcRead');if(b)b.textContent='🔊 Read Again';
}
...
function readCurrent(){
 ...
 u.onend=u.onerror=()=>{speaking=false;currentUtterance=null;const b=$('fcRead');if(b)b.textContent='🔊 Read Again'};
 speechSynthesis.speak(u);
}
function rebuild(){stopRead();list=filters();pos=0;back=false;renderChips();render();if(mode&&list.length)readCurrent()}
function render(){
 ...
 const i=list[pos],e=BANK[i],p=state(i);$('fcCounter').textContent=`${pos+1} / ${list.length}`;$('fcNumber').textContent=`#${e.id}`;$('fcTopic').textContent=e.topic;$('fcPrompt').textContent=e.phrasePrompt;$('fcAnswer').textContent=e.phrase;$('fcSource').textContent=`📘 ${e.bookRef}`;$('fcCategoryBadge').textContent=e.category;$('fcPriorityBadge').textContent=label(p);$('fcPriorityBadge').className=`fc-priority ${p}`;$('fcFace').textContent=back?'EXPLANATION':'QUESTION';card.classList.toggle('show-back',back);$('fcFlip').textContent=back?'Next Question →':'Show Answer →';$('fcTapHint').textContent=back?'Tap card again for the next question.':'Tap card to show the answer.';if(!speaking&&$('fcRead'))$('fcRead').textContent='🔊 Read Again';
}
function cardTap(){
 if(!list.length)return;stopRead();
 if(!back){back=true;render();readCurrent();return}
 pos=(pos+1)%list.length;back=false;render();readCurrent();
}
function move(d){if(!list.length)return;stopRead();pos=(pos+d+list.length)%list.length;back=false;render();if(mode)readCurrent()}
function shuffle(){stopRead();for(let i=list.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[list[i],list[j]]=[list[j],list[i]]}pos=0;back=false;render();if(mode&&list.length)readCurrent()}
...
function install(){
 ...
 <div class="fc-head"><div><h2>🃏 Explanation Flashcards</h2><p>See question → hear question → tap for answer → hear answer → tap for next question.</p></div><b id="fcSetCount"></b></div>
 ...
 <div id="fcCard" class="fc-card" tabindex="0"><div class="fc-meta"><b id="fcFace">QUESTION</b><span id="fcNumber"></span><span id="fcPriorityBadge"></span><span id="fcCategoryBadge" class="fc-category"></span></div><div id="fcTopic" class="fc-topic"></div><div class="fc-front"><div id="fcPrompt" class="fc-prompt"></div></div><div class="fc-back"><div id="fcAnswer" class="fc-answer"></div><div id="fcSource" class="fc-source"></div></div><div id="fcTapHint" class="fc-taphint">Tap card to show the answer.</div></div>
 ...
 <div class="fc-tip"><b>Simple flow:</b> every new card reads itself automatically. Tap the card once for the answer, then tap it again for the next question.</div>`;header.insertAdjacentElement('afterend',panel);
