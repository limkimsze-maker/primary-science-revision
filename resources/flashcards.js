...
let list=[],pos=0,back=false,font=42,mode=false;
...
function rebuild(){list=filters();pos=0;back=false;renderChips();render()}
function render(){
...
function leave(){if(!mode)return;mode=false;existing(true);$('flashcardPanel').classList.add('hide');$('flashcardTab').classList.remove('active')}
function install(){
 ...
 <div class="fc-nav"><button id="fcPrev">← Previous</button><b id="fcCounter"></b><button id="fcFlip" class="primary">✨ Reveal Explanation</button><button id="fcNext" class="next">Next →</button></div>
 <div class="fc-tip"><b>Study idea:</b> say the explanation aloud before flipping the card. Use Category or Study set to concentrate on one area.</div>`;header.insertAdjacentElement('afterend',panel);
