...
function boot(){
  ...
  function gapCue(text){
    ...
    return words.map(w=>{
      ...
      wordNo++;
      const important=/photosynthesis|oxygen|respiration|energy|heat|evaporation|condensation|friction|current|circuit|absorb|transport|competition|continuity|potential|kinetic|germination|fertilisation|pollination|stomata|nutrient|blood|umbilical|light|water|temperature|force/i.test(w);
  ...
  let reviewReturn=null,resumeApp=false,memDraft='',appDraft='';
  ...
  function resetCycle(){stage='flash';flashBack=false;memMode='exact';memNotice='';appPreparedFor=-1;lastChosen=null;reviewReturn=null;resumeApp=false;memDraft='';appDraft=''}
  function applyPicker(keepCurrent=true){
    if(stage!=='flash'||flashBack||reviewReturn)return;
  ...
  const header=document.querySelector('body>.wrap>header');if(!header)return;
  ...
  body.gf-on{background:linear-gradient(180deg,#eef2ff 0,#f8fafc 38%,#f4f7fb 100%)}body.gf-on> .wrap>header .tabs,body.gf-on> .wrap>.controls,body.gf-on> .wrap>.stats,body.gf-on> .wrap>.main-grid,body.gf-on #flashcardPanel{display:none!important}body.gf-on> .wrap{max-width:1120px;padding-top:18px}body.gf-on> .wrap>header{margin-bottom:8px}#guidedFlow{margin:12px auto 28px;max-width:1020px}
  ...
  @media(max-width:850px){.gf-picker{grid-template-columns:1fr 1fr}.gf-searchwrap,.gf-counts{grid-column:1/-1}.gf-counts{justify-content:flex-start}.gf-steps{overflow-x:auto}.gf-step{min-width:110px}.gf-guide .frame-grid{grid-template-columns:1fr}}
  ...
  function updatePicker(){const locked=stage!=='flash'||flashBack||!!reviewReturn;flow.querySelector('.gf-picker').classList.toggle('locked',locked);['gfSearch','gfTopic','gfStatus','gfSort'].forEach(id=>$(id).disabled=locked);$('gfFilterNote').textContent=locked?'Finish or return from this concept step first.':'Choose a topic, search, or show only Known / Not done.';const c=counts();$('gfCounts').textContent=`✅ ${c.known} known · ○ ${c.notdone} not done`}
  ...
  function render(){document.body.classList.add('gf-on');document.body.classList.toggle('gf-classic',classic);flow.dataset.stage=stage;flow.dataset.reviewReturn=reviewReturn||'';flow.dataset.resumeApp=resumeApp?'1':'0';if(classic)return;updatePicker();meta();setSteps();if(empty){$('gfBody').innerHTML='<div class="gf-empty"><h3>No matching concepts</h3><p>Change the topic, search or Known / Not done filter.</p></div>';return}if(stage==='flash')renderFlash();else if(stage==='mem')renderMem();else renderApp()}

  function renderFlash(){
    const e=concept(),b=$('gfBody'),review=!!reviewReturn,returnLabel=reviewReturn==='app'?'Return to Application →':'Return to Memorise →';
    if(!flashBack){b.innerHTML=pills(e)+`<div class="gf-prompt">${esc(e.phrasePrompt)}</div><div class="gf-hint">Answer in your head first.</div><div class="gf-actions"><button id="gfReveal" class="gf-btn gf-primary">Show answer</button>${review?`<button id="gfReturnReview" class="gf-btn gf-muted">${returnLabel}</button>`:''}</div>`;$('gfReveal').onclick=()=>{flashBack=true;render();speak(e.phrase)};if(review)$('gfReturnReview').onclick=returnFromFlash;return}
    b.innerHTML=pills(e)+`<div class="gf-answer">${esc(e.phrase)}</div><div class="gf-source">📘 ${esc(e.bookRef||'Book-backed Science core')}</div><div class="gf-hint">${review?'Review as many times as you need.':'Did you know this explanation?'}</div><div class="gf-actions">${review?`<button id="gfReturnReview" class="gf-btn gf-next">${returnLabel}</button>`:`<button id="gfWrong" class="gf-btn gf-warn">✕ Need practice</button><button id="gfRight" class="gf-btn gf-good">✓ I knew it</button>`}</div>`;
    if(review)$('gfReturnReview').onclick=returnFromFlash;else{$('gfWrong').onclick=()=>finishFlash('wrong');$('gfRight').onclick=()=>finishFlash('right')}
  }
  function finishFlash(result){try{const x=rec(current());x.flashcardResult=result;save()}catch(_e){}stage='mem';memMode='exact';memNotice='';memDraft='';reviewReturn=null;resumeApp=false;flashBack=false;render();setTimeout(()=>$('gfRecall')?.focus(),40)}

  function ladder(){const modes=['exact','gaps','learn'],labels=['Exact recall','Fill blanks','Learn / copy'],at=modes.indexOf(memMode);return `<div class="gf-ladder">${labels.map((l,i)=>`<div class="${i===at?'active':i<at?'passed':''}">${i+1}. ${l}</div>`).join('')}</div>`}
  function renderMem(){
    const e=concept(),b=$('gfBody');let cue='',title='',help='';
    if(memMode==='exact'){title='Exact recall — no hints';help='Type or speak the full Science sentence from memory.'}
    ...
    b.innerHTML=pills(e)+ladder()+notice+`<div class="gf-mode">${title}</div><div class="gf-hint">${help}</div>${cue}<textarea id="gfRecall" class="gf-textarea" spellcheck="false" placeholder="Type the full Science sentence..."></textarea><div class="gf-actions"><button id="gfBackFlash" class="gf-btn gf-muted">← Flashcard</button><button id="gfCheckRecall" class="gf-btn gf-primary">${memMode==='learn'?'Check exact copy':'Check answer'}</button>${resumeApp&&appPreparedFor===current()?'<button id="gfResumeApp" class="gf-btn gf-next">Return to Application →</button>':''}</div><div id="gfMemFeedback"></div>`;
    if(memDraft)$('gfRecall').value=memDraft;
    $('gfBackFlash').onclick=backToFlash;$('gfCheckRecall').onclick=checkRecall;if($('gfResumeApp'))$('gfResumeApp').onclick=returnToApp;
  }
  function checkRecall(){
    const e=concept(),u=$('gfRecall').value,fb=$('gfMemFeedback');if(!norm(u)){fb.innerHTML='<div class="gf-feedback info">Type or speak your answer first.</div>';return}
    const mode=memMode,correct=norm(u)===norm(e.phrase);memDraft=u;
    if(correct){
      ...
      if(resumeApp&&appPreparedFor===current()){fb.innerHTML='<div class="gf-feedback good">✅ Review complete. Returning to the same Application question.</div>';setTimeout(returnToApp,300);return}
    ...
    const next=mode==='exact'?'gaps':'learn';const label=next==='gaps'?'Fill in the blanks':'Learn — exact copy';memMode=next;memNotice=`Not correct yet — support increased automatically to ${label}.`;memDraft='';render();if(next==='learn')speak(e.phrase);setTimeout(()=>$('gfRecall')?.focus(),40);
  }
  ...
  async function chooseApplication(i){
    ...
    if(first)target=pool.find(c=>c.explain)||pool[0];else{const unseen=pool.filter(c=>!history.includes(c.key));target=unseen[0]||pool.slice().sort((a,b)=>history.indexOf(a.key)-history.indexOf(b.key))[0]}
  ...
  async function prepareApp(){stage='app';resumeApp=false;reviewReturn=null;appPreparedFor=-1;lastChosen=null;appDraft='';render();const i=current();try{$('appTab')?.click();const ready=await waitFor(()=>$('aiMarkBtn')&&$('freshAppBtn')&&$('variantNote')&&$('appLiteTip'));if(!ready)throw new Error('Application tools are still loading.');lastChosen=await chooseApplication(i);appPreparedFor=i;render();setTimeout(()=>$('gfAppAnswer')?.focus(),40)}catch(err){$('gfBody').innerHTML=`<div class="gf-feedback bad">Could not prepare the Application question yet.<br><small>${esc(err.message||err)}</small></div><div class="gf-actions"><button id="gfRetryApp" class="gf-btn gf-primary">Try again</button></div>`;$('gfRetryApp').onclick=prepareApp}}
  function renderApp(){
    const b=$('gfBody'),e=concept();if(appPreparedFor!==current()){b.innerHTML=`<div class="gf-loading"><div><div class="gf-spinner"></div>Preparing one Application question for <b>${esc(e.topic)}</b>…</div></div>`;return}
    const q=String($('appQuestion')?.textContent||e.applicationQuestion||'').trim(),tip=$('appLiteTip')?.innerHTML||'Read the command word first and answer only what is asked.',frame=$('appPane')?.querySelector('.frame')?.innerHTML||'',firstNote=(rec(current()).guidedAppHistory||[]).length===1&&lastChosen?.explain?'<div class="gf-feedback info" style="margin:0 0 12px"><b>First Application round:</b> practise D/E → S/R → L/R where this Explain/Why question requires them.</div>':'';
    b.innerHTML=pills(e)+firstNote+`<div class="gf-guide">${tip}</div><div class="gf-prompt" style="text-align:left;font-size:clamp(24px,3.5vw,34px);margin:22px 0">${esc(q)}</div>${frame?`<div class="gf-guide">${frame}</div>`:''}<textarea id="gfAppAnswer" class="gf-textarea" spellcheck="false" placeholder="Write or speak your PSLE Science answer..."></textarea><div class="gf-actions"><button id="gfAppBackFlash" class="gf-btn gf-muted">🃏 Flashcard</button><button id="gfAppBackMem" class="gf-btn gf-muted">← Memorise</button><button id="gfMarkApp" class="gf-btn gf-primary">🤖 Check my answer</button></div><div id="gfAppFeedback"></div>`;
    if(appDraft)$('gfAppAnswer').value=appDraft;$('gfAppBackFlash').onclick=backToFlash;$('gfAppBackMem').onclick=backToMem;$('gfMarkApp').onclick=markApp;
  }
  async function markApp(){const u=$('gfAppAnswer').value.trim(),fb=$('gfAppFeedback'),btn=$('gfMarkApp');if(!u){fb.innerHTML='<div class="gf-feedback info">Write or speak your answer first.</div>';return}appDraft=u;const hidden=$('appAnswer'),ai=$('aiMarkBtn');if(!hidden||!ai){fb.innerHTML='<div class="gf-feedback bad">AI marker is still loading. Try again shortly.</div>';return}hidden.value=u;hidden.dispatchEvent(new Event('input',{bubbles:true}));let before=0;try{before=rec(current()).appAttempts||0}catch(_e){}btn.disabled=true;btn.textContent='🤖 Checking…';fb.innerHTML='<div class="gf-feedback info">AI is checking the Science and command word…</div>';ai.click();const done=await waitFor(()=>{try{return (rec(current()).appAttempts||0)>before||(!ai.disabled&&String($('aiStatus')?.textContent||'').includes('unavailable'))}catch(_e){return false}},35000);btn.disabled=false;btn.textContent='🤖 Check my answer';if(!done){fb.innerHTML='<div class="gf-feedback bad">The AI marker took too long. Please try again.</div>';return}const source=$('appFeedback'),text=source?source.textContent.trim():'Answer checked.';let correct=false;try{correct=rec(current()).appLast==='correct'}catch(_e){}if(correct){try{const x=rec(current());x.guidedCycleCompletedAt=Date.now();x.guidedCycles=(x.guidedCycles||0)+1;save()}catch(_e){}fb.innerHTML=`<div class="gf-feedback good">${esc(text||'✅ Correct')}<br><b>Cycle complete — this concept is now Known.</b> One Application question is enough.</div><div class="gf-actions"><button id="gfNextConcept" class="gf-btn gf-next">Next concept →</button></div>`;$('gfNextConcept').onclick=advance}else fb.innerHTML=`<div class="gf-feedback bad">${esc(text||'Not correct yet.')}<br>Repair this same question and try again. The concept remains Not done until this Application question is correct.</div>`}

  function backToFlash(){
    if(stage==='mem'){memDraft=$('gfRecall')?.value||memDraft;reviewReturn='mem'}
    else if(stage==='app'){appDraft=$('gfAppAnswer')?.value||appDraft;reviewReturn='app'}
    else return;
    try{speechSynthesis.cancel()}catch(_e){}stage='flash';flashBack=false;render();
  }
  function backToMem(){
    if(stage!=='app')return;appDraft=$('gfAppAnswer')?.value||appDraft;try{speechSynthesis.cancel()}catch(_e){}resumeApp=true;reviewReturn=null;stage='mem';memNotice='Review the same concept, then return to the same Application question when ready.';render();setTimeout(()=>$('gfRecall')?.focus(),40);
  }
  function returnFromFlash(){
    const target=reviewReturn||'mem';reviewReturn=null;stage=target;render();if(target==='mem'&&memDraft)$('gfRecall').value=memDraft;if(target==='app'&&appDraft)$('gfAppAnswer').value=appDraft;setTimeout(()=>$(target==='app'?'gfAppAnswer':'gfRecall')?.focus(),40);
  }
  function returnToApp(){
    if(appPreparedFor!==current()){prepareApp();return}try{speechSynthesis.cancel()}catch(_e){}resumeApp=false;reviewReturn=null;stage='app';render();if(appDraft)$('gfAppAnswer').value=appDraft;setTimeout(()=>$('gfAppAnswer')?.focus(),40);
  }
  ...
  window.PSLE_GUIDED_NAV={backToFlash,backToMem,returnFromFlash,returnToApp,getStage:()=>stage,getReviewReturn:()=>reviewReturn};
