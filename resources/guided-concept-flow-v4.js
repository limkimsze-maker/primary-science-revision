...
function boot(){
  ...
  function checkRecall(){
    ...
    const next=mode==='exact'?'gaps':'learn';const label=next==='gaps'?'Fill in the blanks':'Learn — exact copy';memMode=next;memNotice=`Not correct yet — support increased automatically to ${label}.`;render();if(next==='learn')speak(e.phrase);setTimeout(()=>$('gfRecall')?.focus(),40);
  }
